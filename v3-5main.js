...
function mainFunc ($) {
  ...
  function buildPage (varsObj) {
    ...
    $.each(objPageContent, function (theKey, theValue) {
      if (typeof theValue === "string") {
        theValue = theValue.replace(/\s\s\s|\t/g, "");
    ...
    if (!(jQuery("#divQues" + self.quesNum + "-response").length)) {
      ...
      if (IS_PRODUCTION) {
        ...
        const ansToStore = (typeof varsObj === "undefined") ? "n/a" : varsObj.calcTheAns;
    ...
    function respPercCorrect (stuResp, correctAns, rawTolerance) {
      ...
      function percCorrect (respToEvaluate, paramCorrectAns, rawTolerance) {
        ...
        const curCorrectAns = parseFloat(paramCorrectAns);
        let isCorrect;
  ...
  async function loadJSFiles () {
    ...
    const objJS = { "IS_PRODUCTION": IS_PRODUCTION };
