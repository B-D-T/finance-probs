...
function explainPVSinglePmt_PV(qvObj) {
    ...
    // Create a local copy of the qvObj so we can modify it without affecting the original
    let qvExplainer = { ...qvObj };
    ...
    const dispFV = ExplainerHelperFuncs.formatVarPMTForTL(qvExplainer.varFV).toLocaleString('en-US');
    ...
    let myStr = `
        ...
        <p>
            ${tvmtreePVSinglePmt(qvExplainer, objColors)}
        </p>
        <p>
            ${identifyPVVars(qvExplainer, objColors)}
        </p>
        <p>
            ${solvePVSinglePmt_PV(qvExplainer, objColors)}
        </p>
    `;

    return fStrReplaceVarsWithVals(myStr, qvExplainer);
}
...
function explainFVSinglePmt_FV(qvObj) {
    ...
    let myStr = `
        <p>
            You have a lump sum (${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(varPV)}) in year ${varY},
...
function explainFVSinglePmt_N(qvObj) {
    ...
    let myStr = `
        <p>
            You know PV<sub>varY</sub> (${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(varPV)})
...
function explainPVAnnuityConst_PV(qv) {
    ...
    `;
    const annuityValue = fPresentValue({varRate, varPMT, varN});

    let strToReturn = fStrReplaceVarsWithVals(myStr, qvObj);
    const absDiff_VarYmin1_ReturnInYear = Math.abs(varReturnInYear - (varY - 1));

    let explanationToShow = "";
    if (varY - 1 == varReturnInYear) {
        // Do nothing, we're already in the correct year. E.g., varY = 1 and varReturnInYear = 0
        explanationToShow = "";

    } else if (varY - 1  > varReturnInYear) {
        // E.g., delayed annuity, where varY = 12 and varReturnInYear = 8
        const yrsToDiscountBack = absDiff_VarYmin1_ReturnInYear;
        explanationToShow = explainPVSinglePmt_PV({"varFV":annuityValue, varRate, "varY": varY-1, "varN": yrsToDiscountBack, "varReturnInYear": varReturnInYear});

    } else if (varY - 1  < varReturnInYear) {
        // E.g., annuity due, where varY = 0 and varReturnInYear = 0.
        // It would also work if varY = 8 and varReturnInYear = 12, but I don't know when you'd ever use that.
        const yrsToCompoundForward = absDiff_VarYmin1_ReturnInYear;
        explanationToShow = explainFVSinglePmt_FV({"varPV":annuityValue, varRate, "varY": varY-1, "varN": yrsToCompoundForward, "varReturnInYear": varReturnInYear});
    }
    ...
    if (explanationToShow == "") {
        ...
        <p>
            <br>*****************************<br><br>
            The steps above gave us the value in Year ${varY-1},
            but we want PV<sub>${varReturnInYear}</sub>
            (the Present Value of the payments in Year ${varReturnInYear}).
        ...
        <p style="margin-left:30px;">
            "What is the value in Year ${varReturnInYear} of a lump sum payment of
            ${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(annuityValue)}
        ...
        <p>
            ...
            ${varY <= varReturnInYear ? "FV" : "PV"}<sub>${varReturnInYear}</sub>
            of a single payment happening in Year ${varY-1}:
        </p>
        `
        strToReturn += explanationToShow;
    }
...
function explainPVPerpetuityConst_PV(qv) {
    ...
    if (varY == 1){
        ...
        <p style="margin-left:30px;">
            ...
            ${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(theAns)}
            happening in year ${varY-1}, assuming a rate of ${uRound(varRate*100,4)}%?"
        </p>
        <p>
            With that, we can walk through the steps necessary to determine
...
//    const varFV = "??";// qv.varFV || fSetLocalVar("varFV", qv.varFV);
    ...
    let myStr = `
        ...
        <p>
            ${identifyFVGrowingAnn(qvObj, objColors)}
        </p>
        <p>
    ...
    const annuityValue = fFutureValue({varRate, varPMT, varN});
    ...
    } else if (annLastYr < varReturnInYear) {
        // E.g., varY=5, varN=17, varReturnInYear=58
        const yrsToCompoundForward = absDiff_AnnLastYr_ReturnInYear;
        explanationToShow = explainFVSinglePmt_FV({"varPV":annuityValue, varRate, "varY": annLastYr, "varN": yrsToCompoundForward, "varReturnInYear": varReturnInYear});
    
    } else if (annLastYr > varReturnInYear) {
        // E.g., varY=5, varN=17, varReturnInYear=3
        const yrsToDiscountBack = absDiff_AnnLastYr_ReturnInYear;
        explanationToShow = explainPVSinglePmt_PV({"varFV":annuityValue, varRate, "varY": annLastYr, "varN": yrsToDiscountBack, "varReturnInYear": varReturnInYear});
    }

    if (explanationToShow == "") {
        // Do nothing
    } else {
        const strPresentOrFuture = varReturnInYear > annLastYr ? "Future" : "Present";
        const strPVorFV = varReturnInYear > annLastYr ? "FV" : "PV";
        strToReturn += `
        <p>
            <br>*****************************<br><br>
            The steps above gave us the value in Year ${annLastYr},
            but we want PV<sub>${varReturnInYear}</sub>
            (the ${strPresentOrFuture} Value of the payments in Year ${varReturnInYear}).
            At this point, we can (and should)
            <i>completely forget about
            the original ${varN} payments of ${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(varPMT)} --
            they are irrelevant now!</i>
        ...
        <p style="margin-left:30px;">
            "What is the value in Year ${varReturnInYear} of a lump sum payment of
            ${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(annuityValue)}
            happening in year ${annLastYr}, assuming a rate of ${uRound(varRate*100,4)}%?"
        </p>
        <p>
            With that, we can walk through the steps necessary to determine
            ${strPVorFV}<sub>${varReturnInYear}</sub> of a single payment
            happening in Year ${annLastYr}:
        </p>
        `;
        strToReturn += explanationToShow;
    }
...
//    const varFV = "??";// qv.varFV || fSetLocalVar("varFV", qv.varFV);
    ...
    let myStr = `
        ...
        <p>
            ...
            but that's probably not a good use of time.
            As shown on the Decision Tree, we can treat this as a growing annuity.
            ${tvmtreeFVGrowingAnnuity(qvObj, objColors)}
        </p>
        <p>
            ${identifyFVGrowingAnn(qvObj, objColors)}
        </p>
        <p>
            ${solveFVAnnuityGrowing_FV(qvObj, objColors)}
        </p>
    `;
...
function explainPVBondLevelAnnual_PV(qv){
    ...
    const dispTheAns = ExplainerHelperFuncs.numToLocaleStr0to5Digits(uRound(qv.calcTheAns,4));
    myStr+= `
        <div>
...
function timelinePVSinglePmt(qv) {
    ...
    let varN = qv.varN;
    if (!qv.varN) {
        varN = qv.varY ?? fSetLocalVar("varN", qv.varN);
    }
    ...
    let dispFV = qv.varFV;
    if (typeof dispFV === "string" && dispFV.trim() !== "") {
        // Do nothing, keep as is (e.g., "??" or custom formatted number)
    } else {
        try {
            dispFV = ExplainerHelperFuncs.numToLocaleStr0to5Digits(qv.varFV);
        } catch (e) {
            // Do nothing
        }
    }
    ...
    <div style="width:300px;text-align: center; margin:25px;">
        <div style="display:flex; justify-content:center; font-weight:bold;">
            ...
            <div style="width:20%;">${varReturnInYear}</div>
...
function timelineFVSinglePmt(qv) {
    ...
    function getDispTimelineValue(qv, varName) {
        ...
        try {
            dispVal = ExplainerHelperFuncs.numToLocaleStr0to5Digits(varValue);
        } catch (e) {
            // Do nothing
        }
        return dispVal;
...
function timelineAnnuity(qv, tvmType = "pv", annPmts = [], showArrow = true) {
    ...
    if (!(typeof qv.varPMT === "undefined")) {
        varPMT = ExplainerHelperFuncs.numToLocaleStrOto2Digits(qv.varPMT);
    }
    ...
    if (tvmType == "pv") {
        ...
        function getColumnOfFirstPayment() {
            // This finds the start year in the tlYears array
            const startYearIndex = aryTLYears.findIndex(val => val == varY);
            const colNum = startYearIndex;
            return colNum;
        }
        const returnsFVInColumn = countAllPmts - 1;
        const columnOfFirstPayment = getColumnOfFirstPayment();
...
function tvmtreeFVGrowingAnnuity(qv, objColors) {
    const formulaVars = {
        varY: "y",
        varN: "n",
        varRate: "i",
    ...
    const ratesAreEqual = (qv.varRate == qv.varG);
    let myStr = `
        <div>
            <p style="margin-bottom:1em;">
                We're compounding a value forward (i.e., to the right on the timeline),
            ...
            <p>
                This leads us to the formula for the <b>Future Value of a Growing Annuity</b>.
                That formula gives us the FV of the series of payments at the time the last
                payment is made. Since the last payment happens in Year ${qv.varY + qv.varN - 1},
                this returns the value of the annuity in Year ${qv.varY + qv.varN - 1}.<br>
                <span style="font-size:0.8em;">
                    Note that this is a different timing from the formula for the 
                    <i>present</i> value of a growing annuity.
                </span>
            </p>
            <p>
                There are two possible formulas for the FV of a Growing Annuity,
                so we need to ask one more question:
            </p>
            <p style="margin-left:20px; margin-top:1em;">
                Is ${formulaVars.varRate} equal to ${formulaVars.varG}? 
    `;

    if (ratesAreEqual){
        myStr += `<i>Yes, ${formulaVars.varRate} is equal to ${formulaVars.varG}.</i>`;
    } else {
        myStr += `<i>No, ${formulaVars.varRate} is not equal to ${formulaVars.varG}.</i>`;
    }
    ...
    myStr += `
        ...
        <p>
            Now we know which formula to use:
        </p>
    `;
    ...
    if (!ratesAreEqual) {
        ...
        \\[
            varFV_{varY + varN - 1}={varPMT}_{varY} \\left( 
                \\frac{ {(1+varRate)}^varN - (1+varG)^varN }{varRate - varG}
            \\right)
        \\]
        </div>
        `;
...
// Returns HTML for variables in the Present Value values
...
// Returns HTML for variables in the Future Value of a growing annuity
function identifyFVGrowingAnn(qv, objColors) {
    const dispRate = uRound(qv.varRate, 5);
    const dispG = qv.varG ? uRound(qv.varG, 5) : 0;
    let myStr = `
    List the variables in the formula and write what is known and unknown.
    \\[
        \\begin{aligned}
...
function solvePVPerpetuityStand_PV(qv, objColors) {
    ...
    <p>
        ...
        is the same as a lump sum of ${ExplainerHelperFuncs.numToLocaleStrCurrency0to5Digits(theAns)} in year ${varY - 1}.
    </p>
`;
    myStr = addColorToVars(myStr, objColors);
    return fStrReplaceVarsWithVals(myStr, qv);
...
function solveFVAnnuityGrowing_FV(qv, objColors) {
    const varN = qv.varN;
    const varY = qv.varY;
    const varG = qv.varG;
    const dispG = uRound(varG, 4);
    const theAns = fFVGrowingAnnuity(qv);
    const fvInYear = varN + varY - 1;
    let myStr = `
    <p>
        Plug the variables into the formula and solve for the unknown term.
    ...
    `;
    if (uRound(qv.varRate,4) == uRound(qv.varG,4)) {
        myStr += solveFVAnnuityGrowing_FV_RatesEqual(qv, objColors);
    } else {
        myStr += solveFVAnnuityGrowing_FV_RatesUnequal(qv, objColors);
    }

    myStr += `
    <p>
        This can be interpreted as follows:
        The value in year ${fvInYear} of an annuity
        where the first payment (in year ${varY}) 
        is ${ExplainerHelperFuncs.numToLocaleStrCurrency2Digits(uRound(qv.varPMT, 2))},
        and the payment grows at ${uRound(varG*100,4)}% over ${varN} years,
        is the same as a lump sum of
        ${ExplainerHelperFuncs.numToLocaleStrCurrency2Digits(uRound(theAns, 2))} in year ${fvInYear}.
    </p>
    `;
    myStr = addColorToVars(myStr, objColors);
    return fStrReplaceVarsWithVals(myStr, qv);
...
function solveFVAnnuityGrowing_FV_RatesUnequal(qv, objColors) {
    const varPV = qv.varPV;
    const varFV = "FV";
    const varN = qv.varN;
    const varRate = qv.varRate;
    ...
    \\[
        \\begin{aligned}
            FV_{varY + varN - 1} &= {varPMT} \\left( 
                \\frac{ {(1+${dispRate})}^varN - (1+${varG})^varN }{${dispRate} - ${dispG}}
            \\right) \\\\
            {} \\\\
            FV_{${fvInYear}} &= {varPMT} \\left( 
                \\frac{ ${ftvm1Rate(qv)}^varN - ${ftvm1Growth(qv)}^varN }{${ftvmRateSpreadIntrGrowth({ varRate, varG })}}
            \\right) \\\\
            {} \\\\
            FV_{${fvInYear}} &= {varPMT} \\left( 
                \\frac{ ${ftvmFVGrowAnnNetNumer(qv, 5)}}{${ftvmRateSpreadIntrGrowth({ varRate, varG }) }}
            \\right) \\\\
            {} \\\\
            FV_{${fvInYear}} &= {varPMT} \\left( 
                { ${ftvmGrowAnnFVIF({varRate, varN, varG})} }
            \\right) \\\\
            {} \\\\
            FV_{${fvInYear}} &= ${theAns}
        \\end{aligned}
...
function solveFVAnnuityGrowing_FV_RatesEqual(qv, objColor) {
    const varPV = qv.varPV;
    const varFV = "FV";
    const varN = qv.varN;
    const varRate = qv.varRate;
